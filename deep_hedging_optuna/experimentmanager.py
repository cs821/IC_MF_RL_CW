...
class ExperimentManager:
    ...
    def init_optuna_study(self, storage: str = "sqlite:///optuna.db"):
        """初始化Optuna Study"""
        study = optuna.create_study(
            direction="minimize",  # 因为我们希望最小化-Y(0)
            study_name=f"{self.config.algo}_study",
            storage=storage,
            load_if_exists=True,
    ...
    def __init__(self, config):
        ...
        self.exp_dir = f"scd_{config.algo}_experiments/{self.sub_dir}/{config.algo}_{self.exp_time}"
