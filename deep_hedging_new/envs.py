...
class TradingEnv(gym.Env):
    ...
    def __init__(self, cash_flow_flag=0,num_sim=500002, sabr_flag = False,
        ...
        # generate data now
        if sabr_flag:
            self.path, self.option_price_path, self.delta_path, self.bartlett_delta_path = get_sim_path_SABR(tau=init_ttm, n_paths=num_sim,freq=trade_freq)
        else:
            self.path, self.option_price_path, self.delta_path = get_sim_path(tau=init_ttm, n_paths=num_sim, freq=trade_freq)
        ...
        self.observation_space = spaces.Box(
            low=np.array([-np.inf] * self.num_state), 
            high=np.array([np.inf] * self.num_state), 
            dtype=np.float32
        )
